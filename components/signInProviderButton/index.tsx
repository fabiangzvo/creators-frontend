...
export function SignInProviderButton(
  props: SignInProviderButtonProps,
): JSX.Element {
    ...
    switch (provider) {
      case "instagram":
        await authClient.signIn.oauth2({
          providerId: provider,
          callbackURL: `/channels/create/${provider}`,
        });
        break;
      case "youtube":
  ...
  const backgroundIcon = useMemo(() => {
    ...
    return (
      <IconComponent isFilled className={iconVariants({ variant: provider })} />
    );
  }, [provider]);
  ...
  return (
    ...
    >
      ...
      /> */}
      <div className="absolute -bottom-8 -right-8 w-40 h-40 group-hover:opacity-10 group-hover:scale-110 transition-all duration-500 text-primary-300 dark:text-primary-600 pointer-events-none transform rotate-12 opacity-10">
        {backgroundIcon}
      </div>

      <div
        className={iconContainerVariants({
          variant: provider as IconContainerVariants["variant"],
        })}
      >
        {icon}
      </div>
      ...
      <div className="relative z-10 text-center">
        ...
        <p className="text-xs font-medium text-zinc-600 uppercase tracking-widest opacity-0 transition-all duration-500 group-hover:opacity-100 dark:text-zinc-400">
          Conectar Perfil
        </p>
      </div>
    </Card>
